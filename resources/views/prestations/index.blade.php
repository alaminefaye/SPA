...
<h4 class="fw-bold py-3 mb-4">
    <span class="text-muted fw-light">Gestion /</span> Prestations
</h4>
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('prestations.index') }}" method="GET" class="mb-4">
            <div class="row g-3">
                <div class="col-md-8">
                    <div class="input-group">
                        <span class="input-group-text"><i class="bx bx-search"></i></span>
                        <input type="text" name="search" class="form-control" placeholder="Rechercher une prestation par nom..." value="{{ $search ?? '' }}">
                    </div>
                </div>
                <div class="col-md-4">
                    <button type="submit" class="btn btn-primary me-2">Rechercher</button>
                    @if(request()->has('search'))
                        <a href="{{ route('prestations.index') }}" class="btn btn-outline-secondary">Réinitialiser</a>
                    @endif
                </div>
            </div>
        </form>
                ...
                <tbody class="table-border-bottom-0">
                    @forelse($prestations as $prestation)
                        <tr>
                            ...
                            <td>{{ \Carbon\Carbon::parse($prestation->duree)->format('H:i:s') }}</td>
                            <td>
                                <div class="dropdown">
