...
@section('title', 'Modifier Achat #{{ $purchase->id }}')
...
<div class="container-fluid">
    <h1 class="h3 mb-4 text-gray-800">Modifier l'Achat #{{ $purchase->id }}</h1>

    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex justify-content-between align-items-center">
            <h6 class="m-0 font-weight-bold text-primary">Modifier l'achat</h6>
        ...
        <div class="card-body">
            ...
            <form action="{{ route('purchases.update', $purchase) }}" method="POST" id="purchase-form">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="2">{{ old('notes', $purchase->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <table class="table table-bordered" id="products-table">
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td id="total-amount" class="font-weight-bold">{{ number_format($purchase->total_amount, 0, ',', ' ') }} FCFA</td>
                                <td></td>
                            </tr>
                        </tfoot>
                    </table>
...
<script>
    $(document).ready(function() {
        ...
        // Initialiser Select2 pour le champ client
        $('#client_id').select2({
            theme: 'bootstrap-5',
        ...
        @foreach($purchase->items as $index => $item)
            // Créer une ligne pour chaque produit existant
            const existingTemplate = $('#product-row-template').html();
            const existingRow = existingTemplate.replace(/__INDEX__/g, {{$index}});
            $('#products-table tbody').append(existingRow);
            
            // Initialiser la ligne et sélectionner le produit
            const $row = $('#products-table tbody tr:last');
            initRowEvents($row);
            
            // Sélectionner le produit existant
            setTimeout(function() {
                $row.find('.product-select').val('{{$item->product_id}}').trigger('change');
                $row.find('.quantity').val('{{$item->quantity}}').trigger('input');
            }, 100);
        @endforeach
        
        // Si aucun produit existant, ajouter une première ligne de produit
        ...
        $('#purchase-form').submit(function(event) {
            ...
            let isValid = true;
            $('#products-table tbody .product-row').each(function(index) {
                ...
                // Renommer les inputs pour correspondre à l'indice
